...
function FAQItem({ q, a, index }: { q: string; a: string; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="glass rounded-2xl border border-white/8 overflow-hidden hover:border-[#027DFD]/25 transition-colors duration-300"
    >
      <button
      ...
      >
        ...
        <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[#027DFD]/20 flex items-center justify-center">
          {open ? <Minus size={12} className="text-[#13B9FD]" /> : <Plus size={12} className="text-[#13B9FD]" />}
        </span>
      </button>
      <AnimatePresence>
        {open && (
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="section-pad bg-[#030710] relative" ref={ref}>
