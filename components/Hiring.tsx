...
export default function Hiring() {
  return (
    <section id="hiring" className="section-pad mesh-bg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {companies.map((company, i) => (
            <motion.div
              ...
              className="glass rounded-2xl p-6 border border-[var(--card-border)] hover:border-[var(--flutter-blue)]/30 hover:-translate-y-1 transition-all duration-300 group"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[var(--flutter-blue)]/20 to-[var(--flutter-cyan)]/10 flex items-center justify-center border border-[var(--flutter-blue)]/20">
                  <Briefcase size={20} className="text-[var(--flutter-cyan)]" />
                </div>
                <span className="text-[10px] text-[color:var(--foreground)]/35 bg-[var(--glass-bg)] border border-[var(--card-border)] px-2 py-0.5 rounded-full">
                  {company.size} employees
                </span>
              </div>
              <h3 className="text-[var(--foreground)] font-bold text-lg mb-3">{company.name}</h3>
              <div className="space-y-1.5 mb-4">
                {company.roles.map((role) => (
                  <div key={role} className="flex items-center gap-2">
                    <div className="w-1 h-1 rounded-full bg-[var(--flutter-cyan)] flex-shrink-0" />
                    <span className="text-[color:var(--foreground)]/60 text-sm">{role}</span>
                  </div>
                ))}
              </div>
              <button className="text-[var(--flutter-cyan)] text-xs font-semibold flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
