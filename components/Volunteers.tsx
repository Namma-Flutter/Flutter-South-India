        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            {roles.map((role, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-[#027DFD]/15 flex items-center justify-center mb-3">
                  <role.icon size={18} className="text-[#13B9FD]" />
                </div>
                <h3 className="text-white font-bold text-sm mb-1">{role.title}</h3>
                <p className="text-white/45 text-xs leading-relaxed">{role.desc}</p>
              </motion.div>
            ))}
          </div>
          ...
          >
            ...
            <ul className="space-y-2.5 mb-8">
              {perks.map((perk) => (
                <li key={perk} className="flex items-center gap-2.5 text-white/65 text-sm">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#13B9FD] flex-shrink-0" />
                  {perk}
                </li>
              ))}
            </ul>
